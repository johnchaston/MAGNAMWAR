...
RASTtogbk <- function(input_fasta, input_reference, out_name_path) {
    ...
    in_fasta <- seqinr::read.fasta(paste(input_fasta, sep = ""), seqtype = "AA", as.string = T, strip.desc = T)
    
    ## reference file is a .csv file; it should be downloaded from RAST as excel format, saved as a .csv, and read in as a .csv; saved the
    ## tab-delimited version has compatibility problems
    ...
    ## do the steps below, the same as is in the loop but on the first line only, to eliminate an empty space at the top of the file when
    ## doing rbind the first time
    ...
    anot_rast <- merge(anot, in_rast, by.x = "ids", by.y = "feature_id", all = F)
    
    ## STEP3A create the variable to store the unique ID, split off from 'fig': strsplit(...)  STEP3B create the variable of the annotation
    ## as.character(droplevels(anot_rast[1,2]))
    wout <- paste(">gi|NA|ref|", strsplit(as.character(droplevels(anot_rast[1, 1])), split = "|", fixed = T)[[1]][2], "|", as.character(droplevels(anot_rast[1, 
        2])), sep = "")
