...
join_repseq <- function(mcl_data, fa_dir, mcl_mtrx, fastaformat = "new") {
    ...
    try(out_reps <- paste(colnames(mcl_data)[i], "\t", rep_info[[1]][1], "\t", rep_info[[1]][2], "\t", var3[1, 2], sep = ""), T)
    ...
    for (i in 2:dim(mcl_data)[2]) {
        ...
        try(annotations <- paste(colnames(mcl_data)[i], "\t", rep_info[[1]][1], "\t", rep_info[[1]][2], "\t", var3[1, 2], sep = ""), T)
    ...
    for (i in seq(1, length(out_reps), by = 2)) {
        ...
        fa_mtrx[count, ] <- c(info[[1]][1], info[[1]][2], info[[1]][3], info[[1]][4], out_reps[[i + 1]])
        count <- count + 1
    }
