...
#' @param haplo_data output of format_afterOrtho --list of 2 things-- 1: binary matrix indicating the presence / absence of genes in each COG and 2: vector of names of COGs
#' @param var_data R object with column names of the following variables#'
...
#' @param species_name Column name in var_data containing 4-letter species designations
#' @param resp Column name in var_data containing response variable
#' @param fix2 Column name in var_data containing second fixed effect
#' @param rndm1 Column name in var_data containing first random variable
#' @param rndm2 Column name in var_data containing second random variable 
#' @param multi Number of cores (can only be used with surv_multi)
...
analyze_OrthoMCL <- function(haplo_data, var_data, model, species_name, resp=NULL, fix2=NULL, rndm1=NULL, rndm2=NULL, multi=1,time=NULL,event=NULL, time2=NULL, startnum=1, stopnum="end") {  
  
  suppressMessages(library(splitstackshape))
  
  
  haplo_mtrx <- t(haplo_data$pa_matrix)
  colnames(haplo_mtrx) <- NULL
  ...
  if(model == "lmeF2") 
    ...
    mtrx <- suppressMessages(analyze.surv.multi(haplo_mtrx, haplo_names, var_data, species_name, time,event, rndm1,rndm2,multi, startnum, stopnum))
  else if(model == "survmulticensor")
    mtrx <- suppressMessages(analyze.surv.censor.multi(haplo_mtrx, haplo_names, var_data, species_name, time, time2, event, rndm1, rndm2, fix2, multi, startnum, stopnum))
  else 
...
analyze.ffrr <- function(haplo_mtrx, haplo_names, var_data, species_name, resp_var, fix_var2, rndm1_eff, rndm2_eff) {
  ...
  cat("Merging Files\n")
  haplo_tx <- merge(tx, haplo_mtrx, by.y="row.names", by.x=species_name, all=F)
  haplo_tx <- droplevels(subset(haplo_tx, haplo_tx[resp_var] > 0))
    
  num_pdg <- dim(haplo_mtrx)[2] #number of phylogenetic distribution groups
  ...
  sub <- cbind(haplo_tx[,which(colnames(haplo_tx)==resp_var)],haplo_tx[,which(colnames(haplo_tx)==fix_var2)],haplo_tx[,which(colnames(haplo_tx)==rndm1_eff)],haplo_tx[,which(colnames(haplo_tx)==rndm2_eff)])	
  ...
  for(i in 1:(num_pdg)) {  
    ...
    for( j in haplo_names[[i]]) {
      ...
      try(output[out_cnt,] <- c(j, p_val1, p_val1_corrected, p_val2, p_val2_corrected, mean_contain, mean_missing, taxa_contain, taxa_missing),T)
      out_cnt <- out_cnt + 1
    }
  ...
  colnames(output_clean) <- c("COG", "p-val1", "corrected_p-val1", "p_val2", "p_val2_corrected", "mean_COGContain", "mean_COGLack", "taxa_contain", "taxa_miss")
  
  output_clean <- cSplit(output_clean, "COG", sep=",", "long")
  
  return(output_clean)
}

analyze.frr.div <- function(haplo_mtrx, haplo_names, var_data, species_name, resp_var, rndm1_eff, rndm2_eff) {
  ...
  ### create a matrix
  sub <- cbind(haplo_tx[,which(colnames(haplo_tx)==resp_var)], haplo_tx[,which(colnames(haplo_tx)==rndm1_eff)], haplo_tx[,which(colnames(haplo_tx)==rndm2_eff)])	
  sub <- as.data.frame(sub)
  ...
  for(i in 1:(num_pdg)) {  
    ...
    try(p_val_corrected <- p_val*num_pdg, silent = T)
    
    try( if (p_val_corrected > 1) {
      p_val_corrected <- 1
    }, T)
    ...
    ### adding data to output matrix
    for( j in haplo_names[[i]]) {
      try(output[out_cnt,] <- c(j, p_val, p_val_corrected, mean_contain, mean_missing, taxa_contain, taxa_missing),T)
      out_cnt <- out_cnt + 1
    }
  ...
  colnames(output_clean) <- c("COG", "p-val1", "corrected_p-val1", "mean_COGContain", "mean_COGLack", "taxa_contain", "taxa_miss")
...
analyze.frr.plus <- function(haplo_mtrx, haplo_names, var_data, species_name, resp_var, rndm1_eff, rndm2_eff) {
  ...
  ### create a good matrix
  ### changed it to a data frame (2/10/16)
  sub <- cbind(haplo_tx[,which(colnames(haplo_tx)==resp_var)], haplo_tx[,which(colnames(haplo_tx)==rndm1_eff)], haplo_tx[,which(colnames(haplo_tx)==rndm2_eff)])	
  sub <- as.data.frame(sub)
  ...
  for(i in 1:(num_pdg)) {  
    ...
    try(p_val_corrected <- p_val*num_pdg,T)
...
analyze.fr <- function(haplo_mtrx, haplo_names, var_data, species_name, resp_var, rndm_eff) {
  ...
  for(i in 1:(num_pdg)) {  
    ...
    try(p_val1_corrected <- p_val1*num_pdg,T)
    ...
    for( j in haplo_names[[i]]) {
      ...
      try(output[out_cnt,] <- c(j, p_val, p_val1_corrected, mean_contain, mean_missing, taxa_contain, taxa_missing),T)
      out_cnt <- out_cnt + 1
    }
...
analyze.f <- function(haplo_mtrx, haplo_names, var_data, species_name, resp_var) {
...
#   output_clean <- output[rowSums(is.na(output))!=9, ] 
#   colnames(output_clean) <- c("COG", "p-val1", "corrected_p-val1", "p-val2", "corrected_p-val2", "mean_COGContain", "mean_COGLack", "taxa_contain", "taxa_miss")
...
analyze.wilcox <- function(haplo_mtrx, haplo_names, var_data, species_name, resp_var) {
...
analyze.surv.multi <- function(haplo_mtrx, haplo_names, var_data, species_name, time, event, rndm1_eff, rndm2_eff, multi, startnum,stopnum) {
  ...
  ### merge the binary matrix with phenotype data; tx = phenotypes; haplo_mtrx=binary matrix produced by 'parse_orthologGroups'
  cat("Merging Files\n")  
  haplo_tx <- merge(tx, haplo_mtrx, by.y="row.names", by.x=species_name, all=F)
  ...
  sub <- cbind(haplo_tx[,which(colnames(haplo_tx)==time)],haplo_tx[,which(colnames(haplo_tx)==event)],haplo_tx[,which(colnames(haplo_tx)==species_name)],haplo_tx[,which(colnames(haplo_tx)==rndm1_eff)],haplo_tx[,which(colnames(haplo_tx)==rndm2_eff)])	# values in matrix 	###
  ...
  sub$S <- Surv(sub$time,sub$event)	# create the response variable, a survival model
  ...
  ### specify the start and stop positions. 
  # stop number
  if (stopnum == "end") {
  ...
  outmulti <- c(rep(1,7),unlist(foreach(i=icount(stopnum-startnum+1)) %dopar% { 
    ...
    for( j in haplo_names[[i]]) {		
      rm(output2)
      output2 <- try(c(j, p_val1, p_val1_corrected, mean_contain, mean_missing, taxa_contain, taxa_missing),T)
      write(j,file="data.csv")
      write(output2,file=paste("outputs/",j,".csv",sep=""))		  
    }
    as.vector(output)
...
analyze.surv.censor.multi <- function(haplo_mtrx, haplo_names, var_data, species_name, time, time2, event, rndm1_eff, rndm2_eff, fix2, multi,startnum,stopnum) {
  ...
  ### merge the binary matrix with phenotype data: 
  ### tx = phenotypes; 
  ### haplo_mtrx=binary matrix produced by 'parse_orthologGroups'
  ...
  ### make a new phenotype matrix with only the specified data and with assigned names columns (makes the model run better)
  sub3 <- cbind(haplo_tx[,which(colnames(haplo_tx)==time)],
                haplo_tx[,which(colnames(haplo_tx)==time2)],
                haplo_tx[,which(colnames(haplo_tx)==event)],
                haplo_tx[,which(colnames(haplo_tx)==species_name)],
                haplo_tx[,which(colnames(haplo_tx)==rndm1_eff)],
                haplo_tx[,which(colnames(haplo_tx)==rndm2_eff)],
  ...
  sub3$S <- Surv(as.numeric(as.character(sub3$time1)),as.numeric(as.character(sub3$time2)), as.numeric(as.character(sub3$event)))	# create the response variable, a survival model
  ...
  outmulti <- c(rep(1,7),unlist(foreach(i=icount(stopnum-startnum+1)) %dopar% { 
    ...
    for( j in haplo_names[[i]]) {		
      rm(output2)
      output2 <- try(c(j, p_val1, p_val1_corrected, mean_contain, mean_missing, taxa_contain, taxa_missing),T)
      write(i,file="data.csv")
      write(output2,file=paste("outputs/",j,"data.csv",sep=""))
    }
