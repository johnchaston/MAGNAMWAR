  ...
  for(i in 1:length(files)) {
    ...
    if(files[i] != filename & files[i] != "repseq.fasta") {
      ...
      for(j in 1:length(reps_fa)) {
        ...
        #DEPRECATED IF : Ability to save duplicate protein ids in this form: NP_12345_1
        # flawed because what if more than 2 of same id? 
        
        # else {
        #   new_id <- paste(info[[1]][genbnk_id],"_1", sep='')
        #   mcl_info <- paste(id[[1]][1], new_id, sep="|")
        #   info_vec <- c(info_vec, mcl_info)
        #   seq_vec <- c(seq_vec,reps_fa[[j]][1]) 
        # }
        # 
