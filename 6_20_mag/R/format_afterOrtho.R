...
format_afterOrtho <- function(file, format="ortho") {
  ...
  if(format == "ortho") {
    ...
    if (is.null(input$V2)) stop('Probably incorrect format designation: Specified "ortho" (online) and might have meant "groups" (local).')
  ...
  for (j in 1:dim(taxa_protein_COG)[2]) { # for each column COG
    pa_string <- ""
    taxa_in_COG <- c(unique(short[,j]))
    for (i in colnames(pa_mtrx)) { # for each taxa
      if (i %in% taxa_in_COG) {
