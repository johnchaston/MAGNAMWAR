...
pdgplot <- function (data, mcl_matrix, GRP="NONE",  species_colname, data_colname, xlab='Taxa', ylab='Data', ylimit=NULL, tree=NULL, order = NULL) {
  ...
  if (!is.null(tree)) { # order by tree
    ...
    if (!is.null(order)) { # order by order vector
  ...
  ### plotting
  bp <- barplot(x, axes = T, axisnames = T, space = (.2), ylim = c(0,ylimit), las=2, col=col, names.arg=names(x))
...
pdg_v_cog <- function(mcl_data, num=40, ...) {
  ...
  if (nrow(sums) > num) { # if the number of COG limit provided is smaller than the largest number of COGS in PDG
  ...
  ### Plotting
  #dev.off() # reset margins for labels 
  barplot <- barplot(plot$Freq, names.arg=plot$COGS, ylim=c(0,r_axis), xlab = expression(paste("COGs PDG"^"-1")), ylab = "PDGs")
...
phydataerror <- function (phy, data, mcl_matrix, species_colname, data_colname, color=NULL, GRP = NULL, xlabel= 'xlabel', ...) {
  ...
  #### plot bar and phylo
  plottree <- plot(phy, show.tip.label=T, x.lim=.5, cex=.9, label.offset=.002)
  ...
  bp <- barplot(x, width = 1, add = F, horiz = TRUE, offset = x0, axes = FALSE, axisnames = FALSE, space = (.2), xlim=c(0,45), col=col, ...)
...
qqplotter <- function (mcl_mtrx) {
  ...
  plot(e,o,pch=19,cex=.2, xlab=expression(Expected~~-log[10](italic(p))), ylab=expression(Observed~~-log[10](italic(p))), xlim=c(0,max(e)), ylim=c(0,max(o)))
...
manhat_grp <- function (mcl_data, mcl_mtrx, tree=NULL) {
  ...
  ### PROBLEM: WHAT TO DO WHEN finalkey DOESN'T HAVE THE TAXA INCLUDED IN tree?
  #     ex. efOG v efog
  ...
  chk3 <- chk2[chk2[,3]>0,]
    ...
    ###  plotting
  plot <- data.frame(SNP=chk3$Gene, BP=as.numeric(chk3$BP), P=as.numeric(as.character(chk3$PValue)), CHR=as.numeric(chk3$CHR), TAXA=chk3$TAXA)
  plot <- na.omit(plot)
  cat("creating manhattan plot...\n")
  manhattan(plot, chrlabs=as.character(taxa_names$TAXA), las=2, xlab='', cex=.25, suggestiveline = -log10((.05)/dim(mcl_mtrx)[1]), genomewideline= FALSE)
  cat("finished.\n")
  }
